...
std::pair<string, std::variant<int, double, char>> tokenmath::MathTok::parseNumber(){ 
	bool is_float = false;
	string number = "";
	...
	it--;
	if(!is_float){
		return std::make_pair("num", std::stoi(number));
	} else {
		return std::make_pair("num", std::stod(number));
	};
};
...
PairVector tokenmath::MathTok::tokenize(const string* expression) {
	...
	while(it < end) {
		if(std::isdigit(*it)){
			...
			tokens.emplace_back("sub", *it);		
		} else if(*it == '+') {
			tokens.emplace_back("add", *it);
		} else if(*it == '*') {
			tokens.emplace_back("mul", *it);
		} else if(*it == '/') {
			tokens.emplace_back("div", *it);
		} else if(*it == '(') {
			tokens.emplace_back("l", *it);
		} else if(*it == ')') {
			tokens.emplace_back("r", *it);
		};
		it++;
	};
